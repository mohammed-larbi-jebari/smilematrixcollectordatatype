 ...
 *
 */
class smileMatrixType extends eZDataType
{
    const DEFAULT_NAME_VARIABLE = '_smilematrix_default_name_';

    const NUM_COLUMNS_VARIABLE = '_smilematrix_default_num_columns_';
    const NUM_ROWS_VARIABLE = '_smilematrix_default_num_rows_';
    const CELL_VARIABLE = '_smilematrix_cell_';
    const DATA_TYPE_STRING = 'smilematrix';

    /*!
     Constructor
    */
    function smileMatrixType()
    {
        $this->eZDataType( self::DATA_TYPE_STRING, ezpI18n::tr( 'kernel/classes/datatypes', 'Smile Matrix'),
    ...
    */
    ...
    {
        $data = false;
        if ( $http->hasPostVariable( $base . '_smilematrix_cell_' . $contentObjectAttribute->attribute( 'id' ) ) )
            $data = $http->PostVariable( $base . '_smilematrix_cell_' . $contentObjectAttribute->attribute( 'id' ) );
        $count = 0;
    ...
    */
    ...
    {
        $matrix = new smileMatrix( '' );

       $text = $contentObjectAttribute->attribute( 'data_text' );
   
      if($text instanceof smileMatrix) 
      {
          $text = $text->xmlString();
      }
        $matrix->decodeXML( $text );
    ...
    */
    ...
    function customObjectAttributeHTTPAction( $http, $action, $contentObjectAttribute, $parameters )
    {
        ...
        switch ( $action )
        {
            ...
            default :
            {
                eZDebug::writeError( 'Unknown custom HTTP action: ' . $action, 'smileMatrixType' );
            }break;
        }
    }
    ...
    */
    function initializeObjectAttribute( $contentObjectAttribute, $currentVersion, $originalContentObjectAttribute )
    {
        ...
        else
        {
            ...
            $matrix = new smileMatrix( '', $numRows, $contentClassAttribute->attribute( 'content' ) );
    ...
    function fetchClassAttributeHTTPInput( $http, $base, $classAttribute )
    {
        ...
        */
        ...
        $columnNameVariable = $base . '_data_smilematrix_column_name_' . $classAttribute->attribute( 'id' );
        $columnIDVariable = $base . '_data_smilematrix_column_id_' . $classAttribute->attribute( 'id' );
    ...
    */
    ...
    {
        $matrixDefinition = new smileMatrixDefinition();
        $matrixDefinition->decodeClassAttribute( $contentClassAttribute->attribute( 'data_text5' ) );
    ...
    function customClassAttributeHTTPAction( $http, $action, $contentClassAttribute )
    {
        ...
        switch ( $action )
        {
            ...
            case 'remove_selected' :
            {
                ...
                $postvarname = 'ContentClass' . '_data_smilematrix_column_remove_' . $contentClassAttribute->attribute( 'id' );
    ...
    */
    ...
    {
        $rootNode = $attributeNode->getElementsByTagName( 'smileMatrix' )->item( 0 );
    ...
    function batchInitializeObjectAttributeData( $classAttribute )
    {
        ...
        $matrix = new smileMatrix( '', $numRows, $classAttribute->attribute( 'content' ) );
        $db = eZDB::instance();
    ...
    function fetchCollectionAttributeHTTPInput( $collection, $collectionAttribute, $http, $base, $contentObjectAttribute )
    {
        ...
        if ( $http->hasPostVariable( $cellsVarName ) )
        {
            ...
            $matrix->NumRows = count($cells)/$matrix->attribute( 'columnCount' );
            
            $emptyCells = array();
            for ( $i = 0; $i < $matrix->attribute( 'columnCount' ); ++$i )
            {
            ...
            if($matrix instanceof smileMatrix)
            {
                $matrix = $matrix->xmlString();
            }

            $collectionAttribute->setAttribute( 'data_text', $matrix );
...
eZDataType::register( smileMatrixType::DATA_TYPE_STRING, 'smileMatrixType' );
